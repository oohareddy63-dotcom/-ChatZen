...
app.use(cors({
    origin: [
        "http://localhost:5173",
        "http://localhost:5174",
        process.env.FRONTEND_URL || "https://chatzen-frontend.onrender.com"
    ],
    credentials: true
}));
...
async function getAIResponse(messages) {
    ...
    // Try Ollama as last resort (for local development)
    if (process.env.OLLAMA_URL) {
        try {
            console.log("🤖 Using Ollama...");
            const ollamaUrl = process.env.OLLAMA_URL + "/api/chat";
    ...
    throw new Error("No AI service available. Please configure OpenAI, Gemini, or Ollama.");
}
...
app.post("/api/chat", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ 
            ...
            help: "Please ensure OpenAI or Gemini API key is configured",
...
app.listen(PORT, async () => {
    ...
    // Check AI services
    if (!openaiClient && !geminiClient && !process.env.OLLAMA_URL) {
        console.warn("⚠️  WARNING: No AI service configured!");
        console.warn("   Please set OPENAI_API_KEY, GEMINI_API_KEY, or OLLAMA_URL");
    }
});
